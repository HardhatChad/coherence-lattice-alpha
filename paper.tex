...
The form of the embedding weight $w = \Rzero^2/(z(z-1))$ is physically motivated above as the probability that a walker traverses the shared bond coherently, weighted by next-nearest-neighbor path multiplicity. The numerical value $\Rzero^2 = 0.0919$ at $K_{\text{BKT}}$ yields $\delta c_{2\text{vtx}} = 0.0111$, tightly constraining $c$ near the value extracted from CODATA ($c_{\text{exact}} = 2.9859$). A dozen alternative two-vertex weight forms were enumerated in the supplementary script \texttt{two\_vertex\_lce.py}; none produce $\delta c$ consistent with experiment (the nearest alternative gives $\delta c \sim 0.08$). While this enumeration strongly constrains the weight, we do not present the specific choice $\Rzero^2$ as rigorously derived from the lattice LCE formalism. A first-principles derivation of the diamond-lattice linked-cluster embedding weight, producing $\Rzero^2$ as the unique coherence factor for a shared-bond subgraph, remains an open problem. The headline result of this paper---$1/\alpha_{\text{BKT}} = 137.032$ at $29$~ppm---does not depend on this weight; the LCE correction from $137.032$ to $137.035999$ (1.5~ppb) should be read as a convergent plausibility argument until the embedding-weight derivation is completed.
